...
class NnbdominicaPipeline:

    # Database setup
    conn = sqlite3.connect('nbdominica.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nbdominica`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nbdominica WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nbdominica (date, title, link, content)"
